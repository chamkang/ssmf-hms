...
export default function Index({
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('appt.title')}</h2>}
        >
                    ...
                    <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-900/40">
                            <tr className="text-left text-xs uppercase tracking-wide text-gray-500">
                                ...
                                <th className="px-4 py-3">{t('common.patient')}</th>
                                <th className="px-4 py-3">{t('common.doctor')}</th>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {appointments.length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-500">{t('appt.none')}</td></tr>
                            )}
                            {appointments.map((a) => (
                                <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40">
                                    ...
                                    <td className="px-4 py-3"><span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${statusColor[a.status] ?? 'bg-gray-100 text-gray-700'}`}>{t('appt_status.' + a.status)}</span></td>
                                    <td className="px-4 py-3 text-right">
                                        {['pending', 'confirmed'].includes(a.status) && (
                                            <span className="flex justify-end gap-3">
                                                <button onClick={() => router.post(route('flow-board.check-in'), { appointment_id: a.id })} className="text-xs font-medium text-[#0E9F63] hover:underline">{t('appt.checkin')}</button>
                                                <button onClick={() => router.patch(route('appointments.status', a.id), { status: 'cancelled' })} className="text-xs font-medium text-red-500 hover:underline">{t('action.cancel')}</button>
                                            </span>
                                        )}
                                    </td>
                                </tr>
