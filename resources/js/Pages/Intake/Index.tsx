        ...
        <form onSubmit={convert} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                    ...
                    <div className="text-sm text-gray-500">
                        ...
                        {booking.preferred_at && <> · wants {booking.preferred_at.replace('T', ' ').slice(0, 16)}</>}
                    </div>
                    {booking.reason && <div className="mt-0.5 text-sm text-gray-600 dark:text-gray-300">“{booking.reason}”</div>}
                </div>
                <button type="button" onClick={() => router.post(route('intake.reject', booking.id), {}, { preserveScroll: true })} className="text-xs text-red-500 hover:underline">Dismiss</button>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <select className={field} value={data.time} onChange={(e) => setData('time', e.target.value)}>
                    <option value="">{slots.length ? 'Time…' : 'No slots'}</option>
...
export default function Index({ bookings, doctors, services }: { bookings: IntakeBooking[]; doctors: Doctor[]; services: Service[] }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Web bookings</h2>}>
            <Head title="Web bookings" />
            <div className="mx-auto max-w-4xl space-y-4 p-4 sm:p-6 lg:p-8">
                ...
                <p className="text-sm text-gray-500">Bookings submitted on the public website, awaiting confirmation into a slot.</p>
                {bookings.length === 0 ? (
                    <p className="rounded-xl border border-dashed border-gray-300 py-10 text-center text-gray-400 dark:border-gray-600">No pending web bookings.</p>
                ) : (
