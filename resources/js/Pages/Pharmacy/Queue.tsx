...
export default function Queue({ prescriptions }: { prescriptions: Prescription[] }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('nav.pharmacy')}</h2>}>
            <Head title={t('nav.pharmacy')} />
            <div className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
                ...
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-500">{t('pharmacy.queue_title')}</h3>
                    <Link href={route('pharmacy.inventory')} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{t('pharmacy.inventory')}</Link>
                </div>

                <div className="space-y-3">
                    {prescriptions.length === 0 && <p className="rounded-xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-800">{t('pharmacy.nothing')}</p>}
                    {prescriptions.map((p) => (
                        <div key={p.id} className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                            ...
                            <button onClick={() => router.patch(route('pharmacy.dispense', p.id))} className="rounded-md bg-[#0E9F63] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0B7F50]">{t('pharmacy.dispense')}</button>
                        </div>
                    ))}
                </div>
            </div>
