...
export default function Inventory({ batches, drugs, today }: { batches: StockBatch[]; drugs: DrugRow[]; today: string }) {
    ...
    const expiryBadge = (expiry: string | null | undefined) => {
        if (!expiry) return null;
        const days = Math.round((new Date(expiry).getTime() - new Date(today).getTime()) / 86400000);
        if (days < 0) return <span className="rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700">{t('pharmacy.expired')}</span>;
        if (days <= 90) return <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">{t('pharmacy.expiring')} ({days}d)</span>;
        return <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">{t('pharmacy.ok')}</span>;
    };

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('pharmacy.inventory_title')}</h2>}>
            <Head title={t('pharmacy.inventory')} />
            <div className="mx-auto max-w-5xl space-y-5 p-4 sm:p-6 lg:p-8">
                ...
                <div className="flex justify-end">
                    <Link href={route('pharmacy.queue')} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{t('pharmacy.queue_title')}</Link>
                </div>
                ...
                <form onSubmit={submit} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">{t('pharmacy.receive')}</h3>
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                        <div className="sm:col-span-2">
                            <select className={field} value={data.drug_id} onChange={(e) => setData('drug_id', e.target.value)}>
                                <option value="">{t('pharmacy.select_drug')}</option>
                        ...
                        <input className={field} placeholder={t('pharmacy.batch_no')} value={data.batch_no} onChange={(e) => setData('batch_no', e.target.value)} />
                        <input className={field} type="number" placeholder={t('common.quantity')} value={data.quantity} onChange={(e) => setData('quantity', e.target.value)} />
                        <div className="sm:col-span-2">
                            <input className={field} type="date" value={data.expiry_date} onChange={(e) => setData('expiry_date', e.target.value)} />
                        </div>
                        <div className="sm:col-span-2 flex items-end justify-end">
                            <button disabled={processing} className="rounded-md bg-[#0E9F63] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0B7F50] disabled:opacity-50">{t('pharmacy.add_stock')}</button>
                        </div>
                    </div>
                </form>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-900/40">
                            <tr className="text-left text-xs uppercase tracking-wide text-gray-500">
                                <th className="px-4 py-3">{t('common.drug')}</th><th className="px-4 py-3">{t('pharmacy.col_batch')}</th><th className="px-4 py-3">{t('common.quantity')}</th><th className="px-4 py-3">{t('pharmacy.col_expiry')}</th><th className="px-4 py-3"></th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {batches.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-500">{t('pharmacy.no_stock')}</td></tr>}
                            {batches.map((b) => (
