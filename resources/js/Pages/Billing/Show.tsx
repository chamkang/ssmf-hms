...
export default function Show({ invoice }: { invoice: Invoice }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('billing.col_invoice')} {invoice.reference}</h2>}>
            <Head title={invoice.reference ?? t('billing.col_invoice')} />
            <div className="mx-auto max-w-3xl space-y-5 p-4 sm:p-6 lg:p-8">
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <a href={route('billing.receipt', invoice.id)} target="_blank" rel="noreferrer" className="text-sm text-[#0E9F63] hover:underline">{t('billing.print_receipt')}</a>
                    </div>
                    <table className="min-w-full text-sm">
                        ...
                        <tfoot className="text-sm">
                            <tr><td className="pt-2 text-gray-500">{t('common.total')}</td><td className="pt-2 text-right font-semibold">{fcfa(invoice.total)}</td></tr>
                            <tr><td className="text-gray-500">{t('billing.paid_label')}</td><td className="text-right text-green-700">{fcfa(invoice.paid)}</td></tr>
                            <tr><td className="font-semibold text-gray-700 dark:text-gray-300">{t('billing.col_balance')}</td><td className="text-right text-lg font-bold text-[#0A3D62] dark:text-blue-300">{fcfa(invoice.balance)}</td></tr>
                        </tfoot>
                    </table>
                </div>
                ...
                {payments.length > 0 && (
                    <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">{t('billing.payments_title')}</h3>
                        {payments.map((p) => (
                            <div key={p.id} className="flex items-center justify-between gap-2 py-1 text-gray-700 dark:text-gray-300">
                                <span className="flex items-center gap-2">
                                    ...
                                    {p.status === 'pending' && <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">{t('billing.pending')}</span>}
                                    {p.status === 'failed' && <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-800 dark:bg-red-900/40 dark:text-red-200">{t('billing.failed')}</span>}
                                </span>
                                <span className="flex items-center gap-3">
                                    {p.status === 'pending' && (
                                        <button type="button" onClick={() => checkStatus(p.id)} className="text-xs font-medium text-[#0A3D62] hover:underline dark:text-blue-300">{t('billing.check_status')}</button>
                                    )}
                ...
                {settled ? (
                    <div className="rounded-xl border border-green-300 bg-green-50 px-4 py-3 text-sm font-medium text-green-800 dark:border-green-700 dark:bg-green-900/30 dark:text-green-200">{t('billing.fully_paid')}</div>
                ) : (
                    <form onSubmit={submit} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-500">{t('billing.take_payment')}</h3>
                        <div className="mb-3 flex gap-2">
                            {['cash', 'momo'].map((m) => (
                                <button key={m} type="button" onClick={() => setData('method', m)}
                                    ...
                                    {m === 'cash' ? t('billing.cash') : t('billing.momo')}
                                </button>
                            ))}
                        </div>
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div>
                                <label className="text-xs text-gray-500">{t('billing.amount')}</label>
                            ...
                            {data.method === 'cash' ? (
                                <div>
                                    <label className="text-xs text-gray-500">{t('billing.tendered')}</label>
                                ...
                                <>
                                    <div>
                                        <label className="text-xs text-gray-500">{t('billing.momo_phone')}</label>
                                    ...
                                    <div className="sm:col-span-2">
                                        <label className="text-xs text-gray-500">{t('billing.ref_optional')} <span className="text-gray-400">{t('billing.optional')}</span></label>
                                        <input className="mt-1 block w-full rounded-md border-gray-300 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200" value={data.reference} onChange={(e) => setData('reference', e.target.value)} placeholder={t('billing.ref_hint')} />
                                    </div>
                                </>
                            )}
                        </div>
                        {data.method === 'cash' && data.tendered !== '' && (
                            <p className="mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">{t('billing.change_due')} <span className="text-[#0A3D62] dark:text-blue-300">{fcfa(change)}</span></p>
                        )}
                        <div className="mt-4 flex justify-end">
                            <button disabled={processing} className="rounded-md bg-[#0E9F63] px-5 py-2 text-sm font-semibold text-white hover:bg-[#0B7F50] disabled:opacity-50">
                                {data.method === 'momo' ? t('billing.request_momo') : t('billing.record_payment')}
                            </button>
                        </div>
                    </form>
                )}
