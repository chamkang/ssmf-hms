...
export default function Create({ freeBeds }: { freeBeds: Bed[] }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('ipd.admit_title')}</h2>}>
            <Head title={t('ipd.admit_title')} />
            <div className="mx-auto max-w-2xl p-4 sm:p-6 lg:p-8">
                <form onSubmit={submit} className="space-y-5 rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div>
                        <label className={label}>{t('ipd.admitted_at')}</label>
                        <input type="datetime-local" className={field} value={data.admitted_at} onChange={(e) => setData('admitted_at', e.target.value)} />
                    </div>
                    <div>
                        <label className={label}>{t('ipd.reason')}</label>
                        <input className={field} value={data.reason} onChange={(e) => setData('reason', e.target.value)} placeholder="" />
                    </div>
                    <div className="flex items-center justify-end gap-3">
                        <Link href={route('inpatient.board')} className="text-sm text-gray-500 hover:underline">{t('action.cancel')}</Link>
                        <button disabled={processing || !data.patient_id} className="rounded-md bg-[#0E9F63] px-5 py-2 text-sm font-semibold text-white hover:bg-[#0B7F50] disabled:opacity-50">{t('ipd.admit_btn')}</button>
                    </div>
                </form>
            </div>
        </AuthenticatedLayout>
