...
export default function Index({ admissions, status }: { admissions: Admission[]; status: string }) {
    const { t } = useTrans();
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">{t('ipd.admissions_title')}</h2>}>
            <Head title={t('ipd.admissions_title')} />
                    ...
                    <div className="flex gap-1">
                        {STATUSES.map((s) => (
                            <button key={s} onClick={() => router.get(route('inpatient.index'), s === 'all' ? {} : { status: s }, { preserveState: true })}
                                ...
                                {s === 'all' ? t('common.all') : t('ipd_status.' + s)}
                            </button>
                        ))}
                    </div>
                    <div className="flex gap-2">
                        <Link href={route('inpatient.board')} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">{t('ipd.board_title')}</Link>
                ...
                <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <table className="min-w-full text-sm">
                        <thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500 dark:bg-gray-900/40">
                            <tr><th className="px-4 py-3">{t('common.reference')}</th><th className="px-4 py-3">{t('common.patient')}</th><th className="px-4 py-3">{t('ipd.col_bed')}</th><th className="px-4 py-3">{t('ipd.col_admitted')}</th><th className="px-4 py-3">{t('ipd.col_attending')}</th><th className="px-4 py-3">{t('common.status')}</th></tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {admissions.map((a) => (
                                <tr key={a.id} onClick={() => router.visit(route('inpatient.show', a.id))} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-900/40">
                                    ...
                                    <td className="px-4 py-3"><span className={'rounded px-2 py-0.5 text-xs font-medium ' + (a.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200' : 'bg-gray-200 text-gray-700 dark:bg-gray-700 dark:text-gray-200')}>{t('ipd_status.' + a.status)}</span></td>
                                </tr>
                            ))}
                            {admissions.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-400">{t('ipd.none')}</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
